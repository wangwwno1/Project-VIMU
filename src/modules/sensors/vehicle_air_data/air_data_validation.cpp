...
namespace sensors {
    void VehicleAirData::UpdateReferenceState() {
        ...
        if (_reference_states_sub.update(&ref_states)) {
            estimator_offset_states_s offset_states;
            _vehicle_offset_states_sub.copy(&offset_states);
    ...
    void VehicleAirData::ValidateBaroData(const uint8_t &instance, const hrt_abstime &timestamp_sample) {
        ...
        if (ref_baro_ready) {
            ...
            if (_baro_validators[instance] == nullptr) {
                _baro_validators[instance] = new CuSumf (&_baro_hgt_params);
                if (_baro_validators[instance] == nullptr) {
                    delete _baro_validators[instance];
                    _baro_validators[instance] = nullptr;
                    PX4_ERR("Ref Baro validator allocation failed");
                } else {
                    _baro_validators[instance]->reset();
                }
            }

            if (_baro_validators[instance]) {
